...
```{r}
#| label: dvs-6-ex-2
# Lab 9 Bolding column labels for readability
surveys |>
  map_chr(class) |>
...
```{r}
#| label: dvs-6-ex-3
# Lab 9 Arranging the table by the number of missing values
fish |>
  map_int(~sum(is.na(.))) |>
  enframe(name = "Variable", value = "Missing Values") |>
  arrange(`Missing Values`) |>
...
```{r}
#| label: dvs-7-ex-1
# Lab 9 using data_color() with a color palette
fish |>
  map_int(~sum(is.na(.))) |>
  enframe(name = "Variable", value = "Missing Values") |>
  arrange(`Missing Values`) |>
...
```{r}
#| label: pe-1-across
# Lab 8 Using across() inside a custom function to rescale selected columns which makes it work in any case
